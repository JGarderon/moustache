...
pub fn parse<'a>(source: &'a str) -> Result<Vec<Token>,String> {
    ...
    match c {
      ' ' | '\t' | '\n' | '\r' if is_text == false => {
        ...
        let spaceType = match c {
          ...
          _ => panic!(
            "{}",
            format!("Fatal error in program : token space not valid in engine's parser (found {:?})", c)
          )
        };
        stack.push(Token::Space(spaceType));
        portion_start = i+1;
      }
      '(' if is_text == false => {
  ...
  if is_text {
    return Err(format!("text opened at position {} and not closed", portion_start))
  }
